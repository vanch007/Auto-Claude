...
**重要**: 所有用户可见的内容（title, description, rationale, proposedContent 等）必须使用**简体中文**生成。
技术标识符（id, type, 字段名）保持英文。

**IMPORTANT**: Generate all user-facing content (title, description, rationale, proposedContent, etc.) in **Simplified Chinese (简体中文)**.
