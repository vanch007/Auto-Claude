      ...
      {envConfig.githubEnabled && (
        <>
              ...
              <div className="rounded-lg border border-success/30 bg-success/10 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-success">{t('projectSections.github.connectedTo', { repo: 'GitHub CLI' })}</p>
                      {oauthUsername && (
                        <p className="text-xs text-success/80 flex items-center gap-1 mt-0.5">
                          <User className="h-3 w-3" />
                          {t('projectSections.github.authenticatedAs', { username: oauthUsername })}
                        </p>
                      )}
                    </div>
                  </div>
                  <Button
                    ...
                    className="text-xs"
                  >
                    {t('projectSections.github.useManual')}
                  </Button>
                </div>
              </div>
          ...
          {authMode === 'oauth' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('projectSections.github.pat')}</Label>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleSwitchToManual}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    {t('projectSections.github.useOAuth')}
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">
                  {t('projectSections.github.patHelp')}
                  <a
      ...
      <div className="relative">
        ...
        >
          {isLoading ? (
            ...
            <span className="text-muted-foreground">{t('projectSections.github.selectRepo')}</span>
          )}
        ...
        {isOpen && !isLoading && (
          <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-64 overflow-hidden">
            ...
            <div className="p-2 border-b border-border">
              <Input
                placeholder={t('projectSections.github.searchRepos')}
                value={filter}
            ...
            <div className="max-h-48 overflow-y-auto">
              {filteredRepos.length === 0 ? (
                <div className="px-3 py-4 text-sm text-muted-foreground text-center">
                  {filter ? t('projectSections.github.noMatchingRepos') : t('projectSections.github.noReposFound')}
                </div>
              ) : (
...
function RepositoryInput({ value, onChange }: RepositoryInputProps) {
  ...
  return (
    <div className="space-y-2">
      <Label className="text-sm font-medium text-foreground">{t('projectSections.github.repository')}</Label>
      <p className="text-xs text-muted-foreground">
        {t('projectSections.github.repoFormat')}
      </p>
      <Input
    ...
    <div className="rounded-lg border border-border bg-muted/30 p-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {isChecking ? t('projectSections.github.checking') :
              connectionStatus?.connected
                ? t('projectSections.github.connectedToRepo', { repo: connectionStatus.repoFullName })
                : connectionStatus?.error || t('projectSections.github.notConnected')}
          </p>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t('projectSections.github.issuesAvailableDesc')}
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="flex items-center justify-between">
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground pl-6">
          {t('projectSections.github.autoSyncDesc')}
        </p>
      </div>
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground pl-6">
            {t('projectSections.github.defaultBranchDesc')}
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      <div className="relative pl-6">
        ...
        >
          {isLoading ? (
            ...
            <span className="text-muted-foreground">{t('projectSections.github.autoDetect')}</span>
          )}
          <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${isOpen ? 'rotate-180' : ''}`} />
        </button>
        ...
        {isOpen && !isLoading && (
          <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-64 overflow-hidden">
            ...
            <div className="p-2 border-b border-border">
              <Input
                placeholder={t('projectSections.github.searchBranches')}
                value={filter}
                onChange={(e) => setFilter(e.target.value)}
                className="h-8 text-sm"
                autoFocus
