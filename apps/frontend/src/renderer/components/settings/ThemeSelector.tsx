  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3 max-w-md pt-1">
          {(['system', 'light', 'dark'] as const).map((mode) => (
            ...
            >
              {getModeIcon(mode)}
              <span className="text-sm font-medium">{t(`theme.mode.${mode}`)}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">{t('theme.colorThemeDesc')}</p>
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="font-medium text-sm text-foreground">{t(`theme.themes.${theme.id}.name`)}</p>
                  <p className="text-xs text-muted-foreground line-clamp-2">{t(`theme.themes.${theme.id}.description`)}</p>
                </div>
              </button>
            );
          })}
