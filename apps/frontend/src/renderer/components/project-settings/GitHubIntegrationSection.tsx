...
export function GitHubIntegrationSection({
  ...
  return (
    <CollapsibleSection
      title={t('projectSettings.github.sectionTitle')}
    ...
    >
        ...
        <div className="rounded-lg border border-info/30 bg-info/5 p-3 mb-4">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-foreground">{t('projectSettings.github.projectSpecific')}</p>
              <p className="text-xs text-muted-foreground mt-1">
                {t('projectSettings.github.projectSpecificDesc', { project: projectName })}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <Label className="font-normal text-foreground">{t('projectSettings.github.enableIssues')}</Label>
          <p className="text-xs text-muted-foreground">
            {t('projectSettings.github.enableIssuesDesc')}
          </p>
        </div>
        <Switch
        ...
        <>
          ...
          {envConfig.githubAuthMethod === 'oauth' && envConfig.githubToken && !showOAuthFlow ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('projectSettings.github.authentication')}</Label>
                <Button
                  variant="ghost"
                  size="sm"
                ...
                >
                  {t('projectSettings.github.useManualToken')}
                </Button>
              </div>
              <div className="flex items-center gap-2 p-3 rounded-lg border border-success/30 bg-success/5">
                <CheckCircle2 className="h-4 w-4 text-success" />
                <span className="text-sm text-foreground">{t('projectSettings.github.authenticatedViaOAuth')}</span>
              </div>
            </div>
          ) : showOAuthFlow ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('projectSettings.github.personalAccessToken')}</Label>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  ...
                  {t('projectSettings.github.useOAuth')}
                </Button>
              </div>
              <p className="text-xs text-muted-foreground">
                {t('projectSettings.github.createTokenDesc')}{' '}
                <a
                ...
                >
                  {t('projectSettings.github.githubSettings')}
                </a>
              </p>
              <PasswordInput
          ...
          <div className="space-y-2">
            <Label className="text-sm font-medium text-foreground">{t('projectSettings.github.repository')}</Label>
            <p className="text-xs text-muted-foreground">
              {t('projectSettings.github.repositoryDesc')}
            </p>
            <Input
          ...
          {envConfig.githubToken && envConfig.githubRepo && (
            <ConnectionStatus
              ...
              title={t('projectSettings.github.connectionStatus')}
              successMessage={gitHubConnectionStatus?.repoFullName
                ? t('projectSettings.github.connectedTo', { repo: gitHubConnectionStatus.repoFullName })
                : t('common:status.connected')}
              errorMessage={gitHubConnectionStatus?.error || t('projectSettings.github.notConnected')}
          ...
          {gitHubConnectionStatus?.connected && (
            <div className="rounded-lg border border-info/30 bg-info/5 p-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-foreground">{t('projectSettings.github.issuesAvailable')}</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {t('projectSettings.github.issuesAvailableDesc')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <Label className="font-normal text-foreground">{t('projectSettings.github.autoSync')}</Label>
              </div>
              <p className="text-xs text-muted-foreground pl-6">
                {t('projectSettings.github.autoSyncDesc')}
              </p>
            </div>
            <Switch
